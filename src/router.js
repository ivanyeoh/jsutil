...
const findHandler = pathname => {
  const route = routes.find(({ regex }) => pathname.match(regex))
...
const parseHandler = fnOrString => {
  ...
  return () => dispatchEvent(new CustomEvent(PAGE_CHANGE, { detail: { page } }))
}
...
export default {
  add(regex, handler) {
    routes.push({ regex, handler: parseHandler(handler)})
    return this
  },
  navigate(pathname) {
